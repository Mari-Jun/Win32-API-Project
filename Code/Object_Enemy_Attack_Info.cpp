	...
	case Enemy_Type::Bird:
		...
		case Enemy_Status::E_Attack:
			...
			if (enemy.Get_Ani_Count() == 4)
				...
				Create_Hitting_Point(enemy, 50, 25, Hitting_Shape::FRONT, Hit_Owner::HO_Enemy, false, 0, 8, 1.0);
	...
	case Enemy_Type::Zadrom:
		...
		case Enemy_Status::E_Attack:
			...
			if (enemy.Get_Ani_Count() == 8)
				...
				Create_Hitting_Point(enemy, 40, 20, Hitting_Shape::FRONT, Hit_Owner::HO_Enemy, true, 15, 20, 1.0);
			else if (enemy.Get_Ani_Count() == 16)
	...
	case Enemy_Type::Tolpi:
		...
		case Enemy_Status::E_Attack:
			if (enemy.Get_Ani_Count() == 2)
				Create_Hitting_Point(enemy, 100, 50, Hitting_Shape::FRONT, Hit_Owner::HO_Enemy, false, 0, 6, 1.0);
			else if (enemy.Get_Ani_Count() == 16)
				Attack_Enemy_End(enemy, 64);
			break;
		case Enemy_Status::E_SkillQ:
			if (enemy.Get_Ani_Count() == 2)
				Create_Hitting_Point(enemy, 60, 30, Hitting_Shape::FRONT, Hit_Owner::HO_Enemy, true, 10, 20, 1.0);
			else if (enemy.Get_Ani_Count() == 16)
